...
from utils import yaml_config_hook, save_model, generate_random_number
...
def train():
    loss_epoch = 0
    for step, ((x_i, x_j), _, a) in enumerate(data_loader):
        ...
        x_i = x_i.to('cuda')
        x_j = x_j.to('cuda')
        z_i, z_j, c_i, c_j = model(x_i, x_j, X_ii)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss1111.append((loss_epoch / len(data_loader)))
    save_model(args, model, optimizer, args.epochs)
    df = pd.DataFrame(loss1111)
